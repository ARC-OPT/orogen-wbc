...
ConstraintInterface::ConstraintInterface(ConstraintPtr _constraint,
    ...
    if(constraint->config.type == cart){
        ...
        cart_state_out_port = new RTT::OutputPort<base::samples::RigidBodyState>("status_" + cfg.name);
        ...
        jnt_state_out_port = new RTT::OutputPort<base::samples::Joints>("status_" + cfg.name);
    ...
    constraint_out_port = new RTT::OutputPort<Constraint>("constraint_" + cfg.name);
